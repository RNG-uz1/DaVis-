<template>
    <el-row :gutter="2" style="width: 100%; height: 100%">
        <el-col :span="10">
            <div class="inputBox">
                <div class="printTitle">基础K线图</div>
                ...
                <div>
                    ...
                    <el-button type="primary" size="small" style="margin: 20px" @click.native="drawPie">生成图表
                    </el-button>
                </div>
                <div>
                    <div class="input" style="display:block" v-for="item, index in arr">
                        {{ index + 1 }}.
                        <div >
                            横坐标：<el-input v-model="arr[index].xlabel" style="margin-right: 10px"></el-input>
                        </div>
                        <div >
                            起始值：<el-input v-model="arr[index].start" style="margin-right: 10px"></el-input>
                            结束值：<el-input v-model="arr[index].end"></el-input>
                        </div>
                        <div >
                            最小值：<el-input v-model="arr[index].min" style="margin-right: 10px"></el-input>
                            最大值：<el-input v-model="arr[index].max"></el-input>
                        </div>
    ...
    methods: {
        drawPie() {
            ...
            for (let i = 0; i < this.arr.length; i++) {
                xAxisItem.push(this.arr[i].xlabel)
                yAxisItem.push([this.arr[i].start, this.arr[i].end, this.arr[i].min, this.arr[i].max])
            }
            // 绘制图表
            myChart.setOption({
                xAxis: {
